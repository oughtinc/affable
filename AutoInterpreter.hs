...
evaluateExp' match subst = go
    where go varEnv funEnv (Var x) = return $! case M.lookup x varEnv of Just v -> v
          go varEnv funEnv (Value v) = return $ subst varEnv v
            ...
            case M.lookup f altsMap of
                Just alts -> do
                    ...
                    case mMatch of
                        ...
                        Nothing -> matchFailed
                Nothing -> matchFailed
            where matchFailed = do -- TODO: Need to *create* new workspaces on pattern match failure.
    ...
    forkIO $ do
        ...
        case e of -- This is a bit hacky...
            LetFun _ (Call _ _ e) -> do
                t <- evaluateExp' match expandPointers M.empty M.empty (LetFun answerFn e)
